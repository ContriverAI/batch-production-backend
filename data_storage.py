...
def production_data():
    query = "select * from production order by date_time desc, `mixing time` desc;"
    data = pd.read_sql(query, engine)
    return data
...
def get_prod_report(dateto,datefrom,status,product,recall,shift):
    query = "select date_time, product, count(batch), sum(flour), sum(remix), sum(yeast), sum(yield), status, `batch recall` from production where (date_time between '"+datefrom+"' and '"+dateto+"') and status in "+status+" and product in "+product+" and `batch recall` in "+recall+" and shift in "+shift+" group by date_time, product, status, `batch recall`;"
    print("hellolo",query)
    df = pd.read_sql(query, engine)
    return df
